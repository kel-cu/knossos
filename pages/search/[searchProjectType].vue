          ...
          <section aria-label="Category filters">
            <div v-for="(categories, header) in categoriesMap" :key="header">
              ...
              <template v-if="header === 'resolutions'">
                <SearchFilter v-for="category in categories.filter(
                  (x) => x.project_type === projectType.actual
                )" :key="category.name" :active-filters="orFacets" :display-name="$formatCategory(category.name)"
                  :facet-name="`categories:'${encodeURIComponent(category.name)}'`" :icon="null"
              ...
              <template v-else>
                <SearchFilter v-for="category in categories.filter(
                  ...
                  :facet-name="`categories:'${encodeURIComponent(category.name)}'`" :icon="category.icon"
                  @toggle="toggleFacet" />
              </template>
            </div>
          </section>
          <section v-if="projectType.id !== 'resourcepack' && projectType.id !== 'datapack'"
            ...
            <SearchFilter v-for="loader in tags.loaders.filter((x) => {
              ...
              :facet-name="`categories:'${encodeURIComponent(loader.name)}'`" :icon="loader.icon"
              @toggle="toggleOrFacet" />
    ...
    <section class="normal-page__content">
      ...
      <div v-else class="search-results-container">
        <div id="search-results" class="project-list"
          :class="'display-mode--' + cosmetics.searchDisplayMode[projectType.id]" role="list"
          ...
          <ProjectCard v-for="result in results?.hits" :id="result.slug ? result.slug : result.project_id"
            ...
            :created-at="result.date_created" :updated-at="result.date_modified"
            :downloads="result.downloads.toString()" :follows="result.follows.toString()" :icon-url="result.icon_url"
            :client-side="result.client_side" :server-side="result.server_side" :categories="result.display_categories"
            :search="true" :show-updated-date="sortType.name !== 'newest'"
